<!DOCTYPE html>
<html>
<head>
    <meta charset= "utf-8">
    <meta name= "viewport" content="width-device-width, initial-scale=1.0">
    <link rel="stylesheet" type="text/css" href="cvstyle.css"
  
</head>
<link rel="stylesheet" type="text/css" href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
    <body>
        <div class="wrapper">
          <div class="menu-bar">
            <ul>
            <li class "active"> <a href ="index.php">Ana Sayfa</a></li>
                        <li> <a href ="CV.php">Özgeçmiş</a></li>
                        <li> <a href ="Şehrim.php">Şehrim</a></li>
                        <li> <a href ="Mirasimiz.php">Mirasım</a></li>
                        <li> <a href ="iletisim.php">İletişim</a></li>
                        <li> <a href ="ilgialanlarim.php">İlgi Alanlarım</a></li>
                        <li> <a href ="giris.php">Giriş</a></li>
            </ul>
        </div>
            <div class="testimonial">
                <article>
                    <h1> ÖZGEÇMİŞ</h1>
                    <img  src = "ersin.jpg">
                    <div class="icons">
                        <a href="http://www.facebook.com"> <i class= "fa fa-facebook"aria hidden="true"> </i></a>
                        <a href="http://www.twitter.com"> <i class= "fa fa-twitter"aria hidden="true"> </i></a>
                        <a href="http://www.github.com"> <i class= "fa fa-github"aria hidden="true"> </i></a>
                        <a href="http://www.linkedIn.com"> <i class= "fa fa-linkedin"aria hidden="true"> </i></a>
                    
                    </div>
                    <blockquote>
                       Ersin Karakulak-22
                    </blockquote>
                    <h5></h5>
                    <p>EĞİTİM   

                    </p>
                    <div class = "tablo">
                    <table>
                        <tr>
                          <th>Okul</th>
                          <th>Yıl</th>
                          <th>Şehir</th>
                        </tr>
                        <tr>
                          <td>Kızılpınar İlkokulu</td>
                          <td>2006-2013</td>
                          <td>Tekirdağ</td>
                        </tr>
                        <tr>
                          <td>Kızılpınar Anadolu Lisesi</td>
                          <td>2013-2017 </td>
                          <td>Tekirdağ</td>
                        </tr>
                        <tr>
                          <td>Uludağ Üniversitesi/Bilgisayar Mühendisliği</td>
                          <td>2018-2020</td>
                          <td>Bursa</td>
                        </tr>
                        <tr>
                          <td>Sakarya Üniversitesi/Bilgisayar Mühendisliği</td>
                          <td>2020-Devam ediyor.</td>
                          <td>Sakarya</td>
                        </tr>
                        </table>
                     
                        <p>İŞ TECRÜBESİ
                        </p>
                        <table>
                            
                            <tr>
                              <th>Çalıştığı yer                                                                                                  </th>
                              <th>Çalışma tarihi</th>
                              <th>Görevi</th>
                            </tr>
                            <tr>
                              <td>LC Waikiki      </td>
                              <td>Haziran 2020 - Aralık 2020</td>
                              <td>Depo operatörü</td>
                            </tr>
                            <tr>
                              <td>Topdal Plastik</td>
                              <td>Ağustos 2018 - Eylül 2018</td>
                              <td>Enjeksiyon Operatörü</td>
                            </tr> 
                        </table>  
                        <p>DİL YETKİNLİĞİ
                        </p>
                        <table>
                            
                            <tr>
                              <th>Dil</th>
                              <th>Seviyesi </th>
                              <th>Nerede Öğrendiği</th>
                            </tr>
                            <tr>
                              <td>Türkçe                                                                           </td>
                              <td>Ana dil </td>
                              <td>Günlük yaşam</td>
                            </tr>
                            <tr>
                              <td>İngilizce</td>
                              <td>Orta seviye</td>
                              <td>Okul</td>
                            </tr>
                            <tr>
                              <td>Almanca</td>
                              <td>Başlangıç seviyesi</td>
                              <td>Okul</td>
                            </tr>
                           
                      </table>
                      <p>BECERİLER
                    </p>
                    <table>
                        
                        <tr>
                          <th>Kodlama dili</th>
                          <th>Seviyesi</th>
                          <th>Yaptığı proje(varsa)</th>
                        </tr>
                        <tr>
                          <td>C#                                                                                                        </td>
                          <td>Orta seviye</td>
                          <td>Masaüstü form uygulaması</td>
                        </tr>
                        <tr>
                          <td>Java</td>
                          <td>Düşük seviye</td>
                          <td>-</td>
                        </tr>
                        <tr>
                          <td>Pyhton</td>
                          <td>Orta seviyesi</td>
                          <td>Borsa verilerini getiren uygulama</td>
                        </tr>
                      </div>
                  </table>
                     
                        
                       
                  
                </article>
               
            
            </div>
        </div>
        
    </body>


</html>
